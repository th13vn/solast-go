...
abstract contract PoolShareToken is ERC20, Pausable, ReentrancyGuard {
    ...
    function depositWithPermit(
        ...
        IVesperPool(address(token)).permit(
            _msgSender(),
            address(this),
            amount,
            deadline,
    ...
    function withdraw(
        uint256 shares
    ) external virtual nonReentrant whenNotShutdown {
        _withdraw(shares);
    }
    ...
    function withdrawByStrategy(
        ...
        require(
            feeWhiteList.get(_msgSender()) != 0,
            "Not a white listed address"
        );
    ...
    function _handleFee(
        uint256 shares
    ) internal returns (uint256 _sharesAfterFee) {
        if (withdrawFee() != 0) {
    ...
    function _withdrawByStrategy(uint256 shares) internal {
        ...
        _beforeBurning(shares);
        uint256 amount = convertFrom18(
            shares.mul(convertTo18(totalValue())).div(totalSupply())
        );
...
abstract contract VTokenBase is PoolShareToken {
    ...
    function approveToken() external virtual onlyController {
        ...
        IERC20(IStrategy(strategy).token()).safeApprove(
            strategy,
            MAX_UINT_VALUE
        );
    }
     ...
     */
    function _beforeBurning(uint256 /* shares */) internal override {
        IStrategy strategy = IStrategy(controller.strategy(address(this)));
